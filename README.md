...
- **ラベル変更ペア検出**: 座標が近いラベル同士の名称変更候補をシートに出力し、片側のみ存在するラベルも空欄付きで記録。指定したプレフィックスに一致する未変更ラベル一覧を別Excelとして出力
...
2. **オプション設定** (任意):
   ...
   - **ラベル変更ペア抽出時のプレフィックス入力**: `config.txt` の内容を初期値として表示。1行につき1件入力し、未変更ラベル抽出に利用
...
- Excelファイルには以下のシートが含まれます:
  ...
  - **Label Changes シート** (ラベル変更ペア抽出時): 座標が一致するラベル同士の名称変更候補を一覧化
    - `Label A` 列で昇順ソートされ、対応する相手が存在しない場合は反対側の列が空欄になります
  - **未変更ラベルExcel** (ラベル変更ペア抽出時にプレフィックスを指定した場合): `unchanged_labels.xlsx` に各プレフィックスごとのシートを作成し、ペア名・ラベル・個数を昇順に並べて出力します
...
3. **柔軟な出力設定**:
   - カスタマイズ可能な出力ファイル名
   - 色分けされた差分表示
   - ラベル変更ペアや妥当性チェック結果、未変更ラベルを追加シート／ファイルとして出力

## プレフィックス設定

- ルート直下の `config.txt` に、未変更ラベル抽出用のデフォルトプレフィックスを1行ずつ記述します
- アプリのテキストエリアにはこの内容が読み込まれ、必要に応じて一時的に書き換えることができます
- 各プレフィックスは空行を除いた文字列全体が接頭辞として扱われ、ラベルの先頭一致で抽出します
