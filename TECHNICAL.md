...
| 「ラベルが抽出されない」 | TEXTまたはMTEXTエンティティが含まれているか確認。機器符号フィルタリングの設定を確認 |
| 「処理タイムアウト」 | 大きい・複雑なファイルは処理に時間がかかる。ファイルペアの数を減らして試す |
| 「機器符号が認識されない」 | フィルタリングパターンを確認。妥当性チェックを無効にして試す |
...
| バージョン | 内容 |
|-----------|------|
| v1.0.0 | 初回リリース。最大5ファイルペアのサポート、機器符号フィルタリング、機器符号妥当性チェック、Excel形式での色分け出力、自動ファイル名生成 |
