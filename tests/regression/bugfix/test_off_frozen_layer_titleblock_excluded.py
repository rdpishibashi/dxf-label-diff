...
併せて、エンティティ自身のinvisible属性（グループコード60）による除外も
初めて固定する。
...
不具合の識別子: 2026-09-16 ユーザー報告（DXF-diff-manager で先に発覚・修正済み）
    ...
    という指摘に基づき調査したところ、DXF-label-diffは2026-09-11の
    invisible属性除外の横展開（他5プロジェクト）でも対象外になっており、
    `is_invisible()`自体が未実装だったことが判明した。今回entity単位・
    レイヤー単位の両方をまとめて実装した。

以前どう壊れていたか:
    utils/extract_labels.py の収集経路（MODEL_SPACE/PAPER_SPACEの直接配置・
    INSERT自身・INSERT展開後の仮想エンティティ、計4箇所）はいずれも
    invisible属性・レイヤーのオフ/フリーズを一切チェックしておらず、CADソフト
    上で非表示に設定された図形（旧タイトルブロック等）がそのまま抽出結果に
    混入していた。

修正後に保証したいこと:
    - invisible属性が立った直接配置エンティティは除外される。
...
def _build_dxf_with_hidden_old_titleblock(path):
    ...
    - invisible属性が立った直接配置エンティティ 'HIDDEN_DIRECT'
    - 回路ラベル 'R10'（通常レイヤー、枠外）
    """
    doc = ezdxf.new()
    msp = doc.modelspace()
    ...
    # invisible属性が立った直接配置エンティティ（entity単位の除外を確認する）
    hidden_direct = msp.add_text('HIDDEN_DIRECT', dxfattribs={'insert': (200, 0), 'layer': '0'})
    hidden_direct.dxf.invisible = 1
...
def test_off_frozen_layer_titleblock_content_excluded_from_extract_labels():
    with tempfile.TemporaryDirectory() as d:
        ...
        labels, info = extract_labels(path, filter_non_parts=False, sort_order="none")

        # invisible属性が立ったエンティティは含まれない
        assert 'HIDDEN_DIRECT' not in labels, \
            f"invisible属性のエンティティが抽出されている: {labels}"

        # 旧タイトルブロック（off+frozenレイヤー）由来の図番・ラベルは含まれない
        assert 'EE3273-039-90A' not in labels, \
            f"off+frozenレイヤーの旧図番が抽出されている: {labels}"

        # 現行タイトルブロック・回路ラベルは正しく抽出される
        assert 'EE3273-039-90B' in labels
        assert 'R10' in labels

        # APPRV/CHECKは現行タイトルブロック分の1回ずつのみ（旧分が混入していない）
        appvr_count = labels.count('APPRV')
        check_count = labels.count('CHECK')
